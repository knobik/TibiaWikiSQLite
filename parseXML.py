...
if not skipLoading:
    ...
    c.execute('CREATE TABLE Items(id INTEGER PRIMARY KEY AUTOINCREMENT, title STRING, name STRING, vendor_value BIGINT,'
              'actual_value BIGINT, capacity FLOAT, stackable BOOLEAN, image BLOB, category STRING, discard BOOLEAN,'
              'convert_to_gold BOOLEAN, look_text STRING, currency INTEGER)')
    c.execute('CREATE TABLE ItemProperties(itemid INTEGER, property STRING, value STRING)')
    c.execute('CREATE TABLE NPCs(id INTEGER PRIMARY KEY AUTOINCREMENT, title STRING, name STRING, city STRING,'
              'job STRING, x INTEGER, y INTEGER, z INTEGER, image BLOB)')
    ...
    c.execute( 'CREATE TABLE Creatures(id INTEGER PRIMARY KEY AUTOINCREMENT, title STRING, name STRING, health INTEGER,'
               'experience INTEGER, maxdamage INTEGER, summon INTEGER, illusionable BOOLEAN, pushable BOOLEAN,'
               'pushes BOOLEAN, physical INTEGER, holy INTEGER, death INTEGER, fire INTEGER, energy INTEGER,'
               'ice INTEGER, earth INTEGER, drown INTEGER, lifedrain INTEGER, paralysable BOOLEAN, senseinvis BOOLEAN,'
               'image BLOB, abilities STRING, speed INTEGER, armor INTEGER, boss BOOLEAN)')
    c.execute(
        'CREATE TABLE CreatureDrops(creatureid INTEGER, itemid INTEGER, percentage FLOAT, min INTEGER, max INTEGER)')
    c.execute('CREATE TABLE HuntingPlaces(id INTEGER PRIMARY KEY AUTOINCREMENT, name STRING, level INTEGER,'
              'exprating INTEGER, lootrating INTEGER, image STRING, city STRING)')
    ...
    c.execute('CREATE TABLE Spells(id INTEGER PRIMARY KEY AUTOINCREMENT, name STRING, words STRING,element INTEGER,'
              'cooldown INTEGER, premium BOOLEAN, promotion BOOLEAN, levelrequired INTEGER, goldcost INTEGER,'
              'manacost INTEGER, knight BOOLEAN, paladin BOOLEAN, sorcerer BOOLEAN, druid BOOLEAN, image BLOB)')
    c.execute('CREATE TABLE SpellNPCs(spellid INTEGER, npcid INTEGER, knight BOOLEAN, druid BOOLEAN, paladin BOOLEAN,'
              'sorcerer BOOLEAN)')
    c.execute('CREATE TABLE Quests(id INTEGER PRIMARY KEY AUTOINCREMENT, title STRING, name STRING, minlevel INT,'
              'premium BOOLEAN, city STRING, legend STRING)')
    ...
    c.execute('CREATE TABLE Achievements(id INTEGER PRIMARY KEY, name STRING, grade INTEGER, points INTEGER,'
              'description STRING, spoiler STRING, image INT, imagetype INT)')
    c.execute('CREATE TABLE Outfits(id INTEGER PRIMARY KEY AUTOINCREMENT, title STRING, name STRING, premium BOOLEAN,'
              'tibiastore BOOLEAN)')
    c.execute('CREATE TABLE OutfitImages(outfitid INTEGER, male BOOLEAN, addon INTEGER, image BLOB)')
    c.execute('CREATE TABLE Mounts(id INTEGER PRIMARY KEY AUTOINCREMENT, title STRING, name STRING, tameitemid INTEGER,'
              'tamecreatureid INTEGER, speed INTEGER, tibiastore BOOLEAN, image BLOB)')
    ...
    c.execute('CREATE TABLE ConsumableItems(itemid INTEGER, duration INTEGER, costitemid INTEGER,'
              'costitemcount INTEGER)')
...
skip_pages = ["help:", "talk:", "template:", "updates/", "user:", "loot/", "tibiawiki:"]
...
if not skipLoading:
    for child in root.getchildren():
        ...
        if titleTag is None:
            continue
        title = titleTag.text
        if any(s in title.lower() for s in skip_pages):
            continue
        title = formatTitle(title)
        revisionTag = child.find(getTag('revision'))
        if revisionTag is None:
            continue
...
for itemname, tpl in iter(durationCostMap.items()):
    ...
    results = c.fetchall()
    costid = results[0][0]
    c.execute('INSERT INTO ConsumableItems (itemid, duration, costitemid, costitemcount) VALUES (?,?,?,?)',
              (itemid, duration, costid, itemcostcount))
...
c.execute(
    'CREATE TABLE SpellNPCs(spellid INTEGER, npcid INTEGER, knight BOOLEAN, druid BOOLEAN, paladin BOOLEAN, sorcerer BOOLEAN)')
for npcid, spelllist in iter(spells.items()):
    for spell in spelllist:
        ...
        if len(spellresults) > 0:
            ...
            knight = False
            if 'druid' in job:
                druid = spellresults[1]
            elif 'sorcerer' in job:
                sorcerer = spellresults[3]
            elif 'knight' in job:
                knight = spellresults[4]
            elif 'paladin' in job:
                paladin = spellresults[2]
            ...
            c.execute('INSERT INTO SpellNPCs(spellid, npcid, druid, paladin, sorcerer, knight) VALUES (?,?,?,?,?,?)',
                      (spellid, npcid, druid, paladin, sorcerer, knight))
        else:
...
for itemid, npclist in iter(buyitems.items()):
    for npc, value in iter(npclist.items()):
        c.execute('SELECT id FROM NPCs WHERE LOWER(name)=? OR LOWER(title)=? ORDER BY LOWER(title)=? DESC',
                  (npc.strip().lower(), npc.strip().lower(), npc.strip().lower()))
        results = c.fetchall()
        if len(results) > 0:
            npcid = results[0][0]
...
for itemid, currency in iter(currencymap.items()):
    c.execute('SELECT id FROM Items WHERE LOWER(name)=? OR LOWER(title)=? ORDER BY LOWER(title)=? DESC',
              (currency.strip().lower(), currency.strip().lower(), currency.strip().lower()))
    results = c.fetchall()
    if len(results) > 0:
...
for questname, npcs in iter(questNPCs.items()):
    ...
    c.execute('SELECT id FROM Quests WHERE LOWER(name)=? OR LOWER(title)=? ORDER BY LOWER(title)=? DESC',
...
creatureMap = {'bone beast': 'bonebeast', 'crystal crusher': 'crystalcrusher', 'carniphilia': 'carniphila',
               'slime (creature)': 'slime', 'ghouls': 'ghoul', 'orc spearmen': 'orc spearman', 'monks': 'monk',
               'beholder': 'bonelord', 'demon skeletons': 'demon skeleton',
...
for huntingplaceid, creaturelist in iter(huntcreatures.items()):
    for creature in creaturelist:
        ...
        c.execute('SELECT id FROM Creatures WHERE LOWER(name)=? OR LOWER(title)=? ORDER BY LOWER(title)=? DESC',
                  (_creature, _creature, _creature))
        results = c.fetchall()
        if len(results) > 0:
            creatureid = results[0][0]
            c.execute('SELECT huntingplaceid FROM HuntingPlaceCreatures WHERE huntingplaceid=? AND creatureid=?',
                      (huntingplaceid, creatureid))
            if len(c.fetchall()) == 0:
                c.execute('INSERT INTO HuntingPlaceCreatures(huntingplaceid,creatureid) VALUES (?,?)',
                          (huntingplaceid, creatureid))
        else:
            pass
...
itemMap = {'jalape%c3%92o pepper': 'jalapeño pepper', 'jalapeno pepper': 'jalapeño pepper', 'cookies': 'cookie',
           'small rubies': 'small ruby', 'ultimate healing potion': 'ultimate health potion',
           'lethal lissy\'s shirt': 'the lethal lissy\'s shirt',
           'soft boots': 'pair of soft boots', 'brown bag': 'bag', 'gp': 'gold coin',
           'sudden death': 'sudden death rune', 'power bolts': 'power bolt', 'apple': 'red apple',
           'platinum coins': 'platinum coin', 'gold': 'gold coin', 'elephant tusk': 'tusk', 'present box': 'present',
           'rust remover': 'flask of rust remover'}
...
for creatureid, drops in iter(creaturedrops.items()):
    for itemname, dropinfo in iter(drops.items()):
        ...
        if len(results) > 0:
            itemid = results[0][0]
            c.execute('INSERT INTO CreatureDrops(creatureid, itemid, percentage, min, max) VALUES (?,?,?,?,?)',
                      (creatureid, itemid, dropinfo[0], dropinfo[1], dropinfo[2]))
        else:
...
for questid, items in iter(rewardItems.items()):
    for itemname in items:
        ...
        if len(results) > 0:
            ...
            c.execute('SELECT id FROM Outfits WHERE LOWER(name)=? OR LOWER(title)=? ORDER BY LOWER(title)=? DESC',
                      (_item, _item, _item))
            results = c.fetchall()
            if len(results) > 0:
...
pluralMap = {
    ...
    'quara scout': ['quara constrictor scout', 'quara hydromancer scout', 'quara mantassin scout',
                    'quara pincher scout', 'quara predator scout'],
    ...
    'voodoo cultists|cultists': ['acolyte of the cult', 'adept of the cult', 'enlightened of the cult',
                                 'novice of the cult'],
    # 'demons|demonic creatures': [],
    # 'undead': [],
    'rotworms': ['rotworm', 'carrion worm'],
    'outlaws': ['bandit', 'wild warrior', 'assassin', 'hunter', 'smuggler', 'poacher'],
    'barbarians': ['barbarian bloodwalker', 'barbarian brutetamer', 'barbarian headsplitter', 'barbarian skullhunter',
                   'ice witch'],
    'barbarian': ['barbarian bloodwalker', 'barbarian brutetamer', 'barbarian headsplitter', 'barbarian skullhunter',
                  'ice witch'],
...
removeFromDrops = {'demon': ['small stone', 'leather armor', 'bone',
                             'mouldy cheese']}  # demons drop some goblin loot according to the wiki because of the Demon (Goblin) creature
